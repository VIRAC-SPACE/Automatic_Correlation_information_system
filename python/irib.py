...
directory = os.environ['dirrectory'] + exper_name


############# Antab file
cat = "cd " + directory + " &&  cat *antabfs > ./antab"
os.system(cat)
...
contdata = AIPSUVData('CONT','UVDATA',1,1)
linedata = AIPSUVData('LINE','UVDATA',1,1)
...
all_masers = ['G85.411', 'CEPA', 'G24.33+0.14','G028.304-0.3','G029.955-0.0','G030.378-0.2','G030.817-0.0','G031.059+0.0','G033.641-0.2','G034.244+0.1','SERPENSSOUTH','M17','G043.795-0.1','G045.472+0.1','G049.599-0.2','G075.782+0.3','G085.410+0.0','DR21N','DR21C','DR21S']
...
if tmask <= 2:
    ...
    snplt.inext = 'CL'
    snplt.inver =  0 #contdata.table_highver('AIPS PL')
    snplt.dotv = -1
    snplt.nplots = 8
    snplt.optype = 'AMP'
    snplt.go()
    ...
    lwpla.plver = 1
    lwpla.inver = 2 #contdata.table_highver('AIPS PL')
...
if tmask <= 7:
    ...
    for source in masers:
         ...
         gnuplot = "gnuplot -e \"set term postscript eps; set out '" + source + ".LINE_zoom.eps'; set xlabel 'Velocity [km/s]'; set ylabel 'Flux density [Jy]'; set yrange [-1:]; plot '" + sa_spec + "' u 5:(\$6-" + first10 + ") every ::14 w lines title 'Source:" + source + "' \""
         os.system(gnuplot)
         if os.path.exists('Spectra'):
             mv = "mv " + source + ".LINE_zoom.eps Spectra"
