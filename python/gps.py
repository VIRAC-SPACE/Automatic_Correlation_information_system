...
def gps(station, t1, experiment_name, experiment_type):
    x = []
    y = []
    range = 4 * 86400
    ...
    for m in l:
        ...
        filename = "gps." + station.lower()
        if station in ["Ib", "Ir"]:

            print("sh " + os.path.expanduser('~') + "/scripts/get_gps_for_ib_and_ir.sh " + station.lower() + " " +
                  dir + " " + experiment_name + " " + experiment_type)
            os.system("sh " + os.path.expanduser('~') + "/scripts/get_gps_for_ib_and_ir.sh " + station.lower() + " " +
                      dir + " " + experiment_name + " " + experiment_type)
            
            try:
                fp = open("/mnt/VLBI/correlations/" + experiment_type + "/" + experiment_name + "/" + filename)
...
def create_clock_block(vex, fp):
    ...
    if vex['EXPER'][ vex['GLOBAL']['EXPER']]['target_correlator'] == "JIVE":
        exper_type = "sfxc"
    else:
        exper_type = "kana"
    ...
    for station in vex['STATION']:
        ...
        p = gps(station, mid, exper, exper_type)
