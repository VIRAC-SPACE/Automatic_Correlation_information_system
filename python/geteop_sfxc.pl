...
$FINALS_URL = "https://cddis.nasa.gov/archive/vlbi/gsfc/ancillary/solve_apriori/$FINALS_FILE";
...
sub printUsage
{
	...
	print " The EOPS are reformatted so they can be used in a VEX file for SFXC correlation.\n";
        ...
        print "  yyyy-doy-hour-min-sec: The year and day-of-year of the first daily EOP value to obtain\n";
...
#if ($date =~ /(\d{4})-(\d{3})-(\d{2})/)
{
	...
	$min = $4;
	$sec = $5;
	#print "year: $year $doy $hor \n";
        #$jdate = &calcJD($year,$doy, 23, 59, 59);
        $jdate = &calcJD($year,$doy, $hor, $min, $sec);
}
else
{
	&printUsage;
...
# create output
print OUTFILE "    eop_ref_epoch=$year" . "y$eopDay0" . "d$hor" . "h$min" . "m$sec" . "s;\n";
